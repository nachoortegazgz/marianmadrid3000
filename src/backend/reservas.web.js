 ...
 * VERSION: v19.6.15-sdk-mutable-location-payload
...
async function _resolveEditorialAddonOption(optionRef, traceId) {
    ...
    if (cached && Date.now() - cached.timestamp < SERVICE_CACHE_TTL_MS) return cached.data || null;
    ...
    if (!_looksLikeGuid(addonId) || !_looksLikeGuid(groupId) || option?.activo !== true) {
        log.warn("Editorial addon option is not natively bookable", { optionId, traceId });
        return null;
    }
    ...
    _cacheSetBounded(serviceAddonOptionsRAM, optionId, { data: resolved, timestamp: Date.now() }, CACHE_MAX_SIZE);
    return resolved;
}

async function _resolveEditorialAddons(rawOptions, traceId) {
    const refs = Array.isArray(rawOptions) ? rawOptions : [];
    const resolved = await Promise.all(refs.map((option) => _resolveEditorialAddonOption(option, traceId)));
