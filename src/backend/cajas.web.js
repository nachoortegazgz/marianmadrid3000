 ...
 * - v19.0.0-final:
 * - Fixed ERROR_CODES.TRANSACTION_FAIL -> ERROR_CODES.FISCAL_SIGN_FAIL.
 * - Increased ledger mutex TTL to 45000ms to avoid expiry under latency.
 * - Kept idempotency by recordId and structured logging.
 * HISTORIAL:
...
function _toPublicError(err, fallbackCode = "INTERNAL_ERROR", fallbackMessage = "Error interno") {
...
function _normalizeIdPart(v, maxLen = 80) {
...
async function _upsertCurrentCashProjection(diaKey, options = {}) {
    ...
    const keepClosed = isSameDay && current?.estado === CAJA_STATUS.CLOSED && !options.closed;
