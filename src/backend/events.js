 ...
 * Wix eCommerce V2. SSOT for online payment confirmation
 * and idempotent cashbox ledger registrations.
 ...
 * - v18.9.2-ultimate: Header standardized during V2 compliance review.
 * =============================================================================
 */
...
async function _logAuditEvent(tipoEvento, level, message, data = {}, traceId, entityId = 'system') {
    try {
        ...
        await withTimeout(wixData.insert(COLLECTIONS.AUDIT_LOG, { _id: logId, tipoEvento, level, message, data, resourceId: 'SYSTEM', source: 'backend/events.js', fechaLog: new Date(), traceId }, { suppressAuth: true }).catch(() => null), API_TIMEOUT_MS, 'logAuditEvent');
    } catch (err) {
...
async function _updateCitaEstado(bookingId, nuevoEstado, traceId) {
    ...
    try {
        const citaRes = await withTimeout(wixData.query(COLLECTIONS.CITAS).eq('bookingId', bookingId).limit(1).find({ suppressAuth: true }), API_TIMEOUT_MS, 'queryCitaEstado');
        if (citaRes.items?.length > 0) {
            ...
            if (String(cita[CITA_FIELDS.STATUS] || '').toUpperCase() !== String(nuevoEstado || '').toUpperCase()) {
                ...
                await withTimeout(wixData.update(COLLECTIONS.CITAS, updatedCita, { suppressAuth: true }), API_TIMEOUT_MS, 'updateCitaEstado');
            }
        }
    } catch (err) {
...
async function _markCitasRefundedByBookingIds(bookingIds, orderId, refundId, fullyRefunded, traceId) {
    ...
    for (const bookingId of ids) {
        try {
            const res = await withTimeout(wixData.query(COLLECTIONS.CITAS).eq('bookingId', bookingId).limit(1).find({ suppressAuth: true }), API_TIMEOUT_MS, 'queryCitaForRefund');
            const cita = res?.items?.[0];
            if (!cita) continue;
            const meta = cita.meta || {};
            const { _createdDate, _updatedDate, _owner, ...safeCita } = cita;
            const paymentState = fullyRefunded ? 'REFUNDED' : 'PARTIALLY_REFUNDED';
            const updated = {
                ...safeCita,
            ...
            await withTimeout(wixData.update(COLLECTIONS.CITAS, updated, { suppressAuth: true }), API_TIMEOUT_MS, 'updateCitaRefund');
        } catch (e) {
...
async function _markCitasPaidByBookingIds(bookingIds, orderId, traceId) {
    ...
    for (const bookingId of ids) {
        ...
        try {
            ...
            await withTimeout(wixData.update(COLLECTIONS.CITAS, updated, { suppressAuth: true }), API_TIMEOUT_MS, 'updateCitaPaid');
        } catch (error) {
...
async function _markCitasPendingLedgerByBookingIds(bookingIds, orderId, traceId) {
    ...
    for (const bookingId of ids) {
        try {
            const res = await withTimeout(wixData.query(COLLECTIONS.CITAS).eq('bookingId', bookingId).limit(1).find({ suppressAuth: true }), API_TIMEOUT_MS, 'queryCitaForPendingLedger');
            const cita = res?.items?.[0];
            if (!cita) continue;
            const meta = cita.meta || {};
            ...
            await withTimeout(wixData.update(COLLECTIONS.CITAS, updated, { suppressAuth: true }), API_TIMEOUT_MS, 'updateCitaPendingLedger');
        } catch (error) {
...
export async function wixEcom_onOrderPaymentStatusUpdated(event) {
    ...
    try {
        ...
        if (!order || typeof order !== 'object' || orderId === 'unknown') return { status: 'OK' };
        ...
        await recordOnlineInventoryOrderInternal(order, traceId).catch((inventoryError) => {
            log.error('Online inventory mirror failed', { orderId, traceId, error: inventoryError?.message || String(inventoryError) });
        });
        ...
        const bookingIds = bookingLineItems.map((item) => item?.catalogReference?.catalogItemId).filter(Boolean);
        ...
        const existingLedgerRes = await withTimeout(wixData.query(COLLECTIONS.MOVIMIENTOS_CAJA).eq('transactionId', transactionId).limit(1).find({ suppressAuth: true }), API_TIMEOUT_MS, 'checkExistingLedgerPreflight').catch(() => ({ items: [] }));
        if (existingLedgerRes?.items?.length > 0) {
            if (bookingIds.length) await _executeWithRetry(async () => { await _markCitasPaidByBookingIds(bookingIds, orderId, traceId); }, WEBHOOK_RETRIES, WEBHOOK_RETRY_DELAY_MS);
            return { status: 'OK' };
        }

        if (bookingIds.length) await _executeWithRetry(async () => { await _markCitasPendingLedgerByBookingIds(bookingIds, orderId, traceId); }, WEBHOOK_RETRIES, WEBHOOK_RETRY_DELAY_MS);
        ...
        const ledgerRes = await registerBookingPayment(linkedBookingIds || null, finalLedgerAmount, FORMA_PAGO.ONLINE, { concept: `Order paid ${orderId}`, resourceId: 'online', traceId, transactionId, orderId, origen: 'WIX_ECOM_PAYMENT_WEBHOOK', tipoMovimiento: TIPO_MOVIMIENTO.VENTA_ONLINE });
        ...
        await queueFiscalRecovery({
            ...
            concept: `Order paid ${orderId}`,
            resourceId: 'online',
            tipoMovimiento: TIPO_MOVIMIENTO.VENTA_ONLINE,
            traceId,
            lastError: ledgerRes?.error?.message || 'LEDGER_REGISTRATION_FAILED',
        });
        await _logAuditEvent('LEDGER_REGISTRATION_FAILED', 'ERROR', `Ledger registration queued for order ${orderId}`, { orderId, bookingIds, ledgerError: ledgerRes?.error || 'Unknown error', traceId }, traceId, orderId);
        return { status: 'OK' };
    } catch (error) {
        const normalized = _handleError(error, 'wixEcom_onOrderPaymentStatusUpdated', traceId);
        await _logAuditEvent('WEBHOOK_CRITICAL_ERROR', 'ERROR', `Critical error in webhook: ${normalized.message}`, { error: normalized.message, traceId }, traceId);
        return { status: 'OK' };
    }
}
...
export async function wixEcom_onOrderRefunded(event) {
    ...
    try {
        const orderId = event?.orderId || event?.order?._id || 'unknown';
        ...
        const originalMovementRes = await withTimeout(wixData.query(COLLECTIONS.MOVIMIENTOS_CAJA).eq('transactionId', originalTransactionId).limit(1).find({ suppressAuth: true }), API_TIMEOUT_MS, 'queryOriginalMovement').catch(() => ({ items: [] }));
        ...
        const linkedBookingIds = originalMovement.reservaIdVinculada ? String(originalMovement.reservaIdVinculada).split(',').filter(Boolean) : [];
        ...
        const ledgerRes = await registerBookingPayment(linkedBookingIds.join(',') || null, -refundAmount, FORMA_PAGO.ONLINE, {
            concept: `Refund - Order ${orderId}`,
            resourceId: 'online',
            traceId,
            transactionId,
        ...
        const refundedTotal = (refundsRes?.items || []).reduce((sum, movement) => sum + Math.abs(Number(movement?.importeContable || movement?.importeTotal || 0)), 0);
...
export async function wixEcom_onOrderCanceled(event) {
    ...
    try {
        ...
        if (orderId !== 'unknown') {
            const lineItems = Array.isArray(order.lineItems) ? order.lineItems : [];
            const bookingsAppId = APP_IDS.BOOKINGS;
            const bookingIds = lineItems.filter((item) => item?.catalogReference?.appId === bookingsAppId).map((item) => item?.catalogReference?.catalogItemId).filter(Boolean);
            for (const bId of bookingIds) { await _updateCitaEstado(bId, ESTADO_CITA.CANCELED, traceId); }
        }
        return { status: 'OK' };
    } catch (error) {
