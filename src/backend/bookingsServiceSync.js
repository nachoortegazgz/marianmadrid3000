 ...
 *                 owned by Wix.
 * STANDARDS: G10 ASCII Strict. No customer data or raw upstream errors in logs.
 * =============================================================================
 */
...
export async function enqueueBookingsServiceSync(sourceItem) {
    ...
    const queue = {
        _id: _queueId(sourceItem._id),
        serviceId: String(sourceItem.serviceId || "").trim(),
        sourceItemId: String(sourceItem._id || "").trim(),
