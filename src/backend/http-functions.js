 ...
 * VERSION: v19.6.2-serviceid-linkfases-contract
 * RESPONSIBILITY: Public HTTP REST endpoints for external integrations (M365 / Power Automate).
 * HARDENED: rate limiter surface-scoped, auth error meta.reason.
...
export async function get_getAvailability(request) {
    ...
    try {
        // --- CORREGIDO: rateLimiter con surface y key ---
        ...
        const serviceIdRaw = _safeTrim(query.serviceId);
        const resourceId = query.resourceId ? _safeTrim(query.resourceId) : null;
        const dateYMD = _safeTrim(query.dateYMD || query.date);
