 ...
 * VERSION: v19.6.7-ready-only-context
...
$w.onReady(async function () {
    ...
    createWidgetBridge(widget, {
        ...
        onContextReady: async () => {
            // IMPORTANT: webMethod signature is (slugOrId) only.
            ...
            return {
                ...
                metadata: {
                    ...
                    imageUrl: _safeTrim(metadata.imageUrl) || UI.DEFAULT_SERVICE_IMAGE_URL,
                    pricing: {
                        base: precio,
        ...
        onWidgetMessage: async (msg, post) => {
            ...
            if (type === String(MESSAGE_TYPES.BOOK).toUpperCase()) {
                ...
                const requestedAddonIds = Array.isArray(incomingMeta.addons) ?
                    incomingMeta.addons.map((addon) => _safeTrim(addon?.addonId)).filter(Boolean) : [];
                const canonicalAddons = (currentServicePresentation?.addons || [])
                    .filter((addon) => requestedAddonIds.includes(addon.addonId));
                ...
                const metaCita = {
                    ...
                    uiPairToken: _safeTrim(incomingMeta.uiPairToken || "") || null,
                    pairToken: _safeTrim(incomingMeta.pairToken || "") || null,
                };
                        ...
                        try {
                            ...
                            await wixWindowFrontend.openLightbox("ConfirmacionReserva", {
                                ...
                                extras: canonicalAddons.length > 0 ?
                                    canonicalAddons.map((addon) => addon.nombre).join(", ") :
                                    "Ninguno",
